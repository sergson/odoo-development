...
class ReportGeneralLedgerPartner(models.AbstractModel):
    _name = 'report.account_extra_reports_rm.report_generalledger_partner'

    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account, partner):
        """
        :param:
...
# DOWN LINE MODIFED BY ME
            ...
            sql = ("SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, NULL AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
                ...
                WHERE l.account_id IN %s AND l.partner_id = %s" + filters + ' GROUP BY l.account_id ')
            params = (tuple(accounts.ids), partner.id) + tuple(init_where_params)
        ...
        sql = ('SELECT l.id AS lid, l.account_id AS account_id, l.date AS ldate, j.name AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            ...
            JOIN account_journal j ON (l.journal_id=j.id)\
            JOIN account_account acc ON (l.account_id = acc.id)\
            WHERE l.account_id IN %s AND l.partner_id = %s ' + filters + ' GROUP BY l.id, l.account_id, l.date, j.name, l.currency_id, l.amount_currency, l.ref, l.name, m.name, m.id, c.symbol, p.name, acc.code ORDER BY ' + sql_sort)
        params = (tuple(accounts.ids), partner.id) + tuple(where_params)
        cr.execute(sql, params)
...
# my add end
    ...
    def render_html(self, data):
        ...
        partner = self.env['res.partner'].browse(data['form'].get('partner'))
...
#MY ADD END
        accounts_res = self.with_context(data['form'].get('used_context',{}))._get_account_move_entry(accounts, init_balance, sortby, display_account, partner)
        docargs = {
            'doc_ids': self.ids,
        ...
        return self.env['report'].render('account_extra_reports_rm.report_generalledger_partner', docargs)
