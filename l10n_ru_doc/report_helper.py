...
#            numeration = re.findall('\d+$', name)
            numeration = re.findall('\d+/\d+$', name)
...
#MY MOD START
        if partner.country_id: repr.append(partner.country_id.name)
        if partner.state_id: repr.append(partner.state_id.name) 
#MY MOD END 
